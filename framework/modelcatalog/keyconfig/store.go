...
type Store struct {
	// mu serializes Replace / SetProvider / RemoveProvider so the two-phase
	// Replace (upsert pass + cleanup pass) cannot race with an interleaved
	// SetProvider whose newly-stored provider would otherwise be misidentified
	// as "absent from snapshot" by the cleanup pass and silently dropped.
	// Readers continue to use sync.Map's lock-free path and do not acquire it.
	mu sync.Mutex
...
// previous state but absent from snapshot are dropped. Each provider's state
// is swapped atomically, but the overall operation across all providers is
// not atomic — concurrent readers may observe a mix of old and new state
// during the brief window of a Replace call. Use on initial load and on full
// cross-pod resyncs.
