...
// migrationAddUserAgentColumn adds the user_agent column to the logs table.
// It stores the raw HTTP User-Agent of the calling client verbatim; the UI maps
// it to a client app (Claude Code, Codex, Cursor, ...) for display and filtering.
...
// migrationAddUserAgentColumnToMCPToolLogs adds the user_agent column to the
// mcp_tool_logs table, mirroring migrationAddUserAgentColumn for MCP tool calls.
...
// rebuild of mv_logs_hourly (now grouped by user_agent) and the creation of
// mv_filter_user_agents happen on the next startup via ensureMatViews /
